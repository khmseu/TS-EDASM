...
START NOP
    ...
    `);
    const test1Pass =
      !test1.ok && test1.errors.some((e) => /range|branch/i.test(e));
    if (test1Pass) {
    ...
    const test2Pass =
      test2.ok && valueResult === 0x42 && typeof valueResult === "number";
    if (test2Pass) {
...
START NOP
    ...
    if (branchesPass === mnemonics.length) {
      log(
        `✓ PASS: All ${mnemonics.length} branch instructions detected correctly`,
      );
      passCount++;
    } else {
      log(
        `✗ FAIL: Only ${branchesPass}/${mnemonics.length} branch instructions passed`,
      );
      failCount++;
    }
    log("");
