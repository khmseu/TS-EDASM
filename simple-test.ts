...
console.log('Test 1 (LDA #$42):', {
  ok: test1.ok,
  errors: test1.errors,
  objectBytes: test1.artifacts?.objectBytes?.slice(0, 5).map(b => '0x' + b.toString(16).toUpperCase().padStart(2, '0')).join(', ')
});
...
console.log('\nTest 2 (BNE START):', {
  ok: test2.ok,
  errors: test2.errors,
  objectBytes: test2.artifacts?.objectBytes?.slice(0, 5).map(b => '0x' + b.toString(16).toUpperCase().padStart(2, '0')).join(', ')
});
