...
export function link(objects: Uint8Array[], options: LinkOptions = {}): LinkResult {
  const errors: string[] = [];
  ...
  for (let i = 0; i < objects.length; i++) {
    try {
      ...
      errors.push(`Failed to parse object file ${i}: ${err instanceof Error ? err.message : String(err)}`);
    }
  }
