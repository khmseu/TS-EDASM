      ...
      try {
        ...
        if (labelName.startsWith(".")) {
          if (!state.lastGlobalLabel) {
            state.errors.push(
              `Line ${state.lineNumber}: Local label ${labelName} without preceding global label`,
            );
            continue;
          }
        ...
        state.errors.push(
          `Line ${state.lineNumber}: ${err instanceof Error ? err.message : String(err)}`,
        );
      }
    }
...
function processDirective(
  ...
  switch (directive) {
    ...
    case "DCI": {
      ...
      if (!line.operand) {
        state.errors.push(
          `Line ${state.lineNumber}: ${directive} requires string operand`,
        );
        return;
      }
