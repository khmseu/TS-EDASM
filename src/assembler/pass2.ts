...
export function pass2(
  ...
  for (const line of lines) {
    ...
    // Handle directives
    if (isDirective(mnem)) {
      const bytesGenerated = processDirective(
        buffer,
        line,
        mnem,
        evaluator,
  ...
  switch (directive) {
    ...
    case "DFB": {
      ...
      for (const val of values) {
        ...
        if (result.undefined) {
          errors.push(
            `Line ${line.lineNumber}: Undefined value in ${directive}`,
          );
          generated.push(0);
        } else {
...
function generateInstruction(
  ...
  if (opcode === null) {
    return {
      error: `Invalid addressing mode ${mode} for mnemonic ${mnemonic}`,
    };
  }
...
function extractOperandValue(
  ...
  if (mode === AddressingMode.IMMEDIATE) {
    ...
    mode === AddressingMode.INDIRECT_ZP ||
    mode === AddressingMode.INDIRECT_ABS_X
  ) {
    expr = expr.replace(/^\(|\)$/g, "");
