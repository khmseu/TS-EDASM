...
function showHelp(): void {
  ...
  console.log(
    "  --origin <addr>     Set assembly origin address (hex or decimal)",
  );
  ...
  console.log(
    "  - ProDOS file attributes are stored in JSON dotfiles (.filename)",
  );
    ...
    if (arg === "-h" || arg === "--help") {
      ...
      if (i + 1 < args.length) {
        ...
        if (cpu !== "6502" && cpu !== "65C02") {
          console.error(
            `Error: Invalid CPU type: ${cpu} (expected 6502 or 65C02)`,
          );
          process.exit(1);
        }
        options.cpu = cpu;
...
async function runAsm(args: string[]): Promise<number> {
  ...
  if (inputs.length > 1) {
    console.error(
      "Error: Multiple input files not supported (use linker for multi-file projects)",
    );
    return 1;
  }
  ...
  try {
    if (result.artifacts.objectBytes) {
      ...
      if (options.verbose) {
        console.log(
          `Object file written: ${outPath} (${result.artifacts.objectBytes.length} bytes)`,
        );
      }
    }
...
async function runLink(args: string[]): Promise<number> {
  ...
  try {
    if (result.artifacts.executable) {
      ...
      if (options.verbose) {
        console.log(
          `Executable written: ${outPath} (${result.artifacts.executable.length} bytes)`,
        );
      }
    }
